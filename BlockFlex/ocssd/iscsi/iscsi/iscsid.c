...
{
    //printf("ZZZZZZ static int iscsi_target_cmd_queue(struct iscsi_task *task)\n");
	struct scsi_cmd *scmd = &task->scmd;
	struct iscsi_connection *conn = task->conn;
	struct iscsi_cmd *req = (struct iscsi_cmd *) &task->req;
	uint32_t data_len;
    ...
    //printf("ZZZ err = target_cmd_queue(conn->session->target->tid, scmd);\n");
...
{
    //printf("ZZZZZZ static int iscsi_task_execute(struct iscsi_task *task) \n");
	...
	case ISCSI_OP_SCSI_CMD:
	    ...
	    //printf("ZZZ case ISCSI_OP_SCSI_CMD:\n");
        //printf("ZZZ iscsi_scsi_cmd_execute(task);\n");

		err = iscsi_scsi_cmd_execute(task);

        /*added by mai*/ 
        /*struct iscsi_task fake_iscsi_task;
        printf("ZZZ sizeof(struct iscsi_task) = %d\n", sizeof(fake_iscsi_task));
        printf("ZZZ sizeof(struct scsi_cmd) = %d\n", sizeof(task->scmd));
        */
