...
struct scsi_data_buffer {
	int resid;
	uint32_t length;
	uint64_t buffer; //mai: pointer of the buffer
};
...
struct scsi_cmd {
	...
	uint64_t offset;
	uint32_t tl;
	uint8_t *scb;//mai: real scsi cmd CDB
	int scb_len;
	uint8_t lun[8];
	int attribute;
	uint64_t tag;
...
//mai: scsi_set_result(), scsi_get_result(), ...
...
//mai: scsi_get_in_buffer(), scsi_get_out_buffer()
