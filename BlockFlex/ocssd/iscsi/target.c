				  ...
				  char *path)
{
	...
	lu->fd = dev_fd;
	lu->addr = 0;
	// FIXME: set it to a big capacity; we are directing requests to the OCSSD anyway
	lu->size = (uint64_t) 1 << 40; // size;
	lu->path = path;
...
/*mai: check cmd arribute to see if it's enabled or not*/
...
{
    //printf("PTE:PTE: int target_cmd_queue(int tid, struct scsi_cmd *cmd)\n");
	struct target *target;
	struct it_nexus *itn;
	 ...
	 */
   // printf("PTE: return cmd->dev->cmd_perform(tid, cmd);\n");
...
{
    // printf("PTE:PTE: int target_cmd_perform(int tid, struct scsi_cmd *cmd)\n");
	...
	if (enabled) {
        //printf("PTE: scsi_cmd_perform(cmd->it_nexus->host_no, cmd)\n");
