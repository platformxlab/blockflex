...
{
    // printf("PTE:PTE: int scsi_cmd_perform(int host_no, struct scsi_cmd *cmd): *cmd= %p, cmd->scb[0]= %x \n", cmd, cmd->scb[0]);
    ...
    //mai:
    //cmd->dev: struct scsi_lu *dev;
    //scsi_lu.dev_type_template: struct device_type_template dev_type_template
    // printf("PTE: return cmd->dev->dev_type_template.ops[op].cmd_perform(host_no, cmd);\n");
