...
public class google_matcher {
    private static void print_util(List<vssd> vssds, HashMap<String, Double> allocs, String filename){
        ...
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(filename))))) {
            for(String k : util_per_vm.keySet()){
                writer.write(k+" "+(1-util_per_vm.get(k)/vssd_per_vm.get(k)/allocs.get(k))+" "+(1-max_util_vm.get(k)/allocs.get(k)));
    ...
    public static void main(String[] args) throws FileNotFoundException ,IOException{
        ...
        while(line!=null) {
            ...
            double start = Double.parseDouble(ss[1]);
            double end = Double.parseDouble(ss[2]);
            double alloc = Double.parseDouble(ss[4]);
            String id = ss[0];
            double len = end - start;
        ...
        print_util(vssds, allocs, "original_util.txt");
        int tot_reqs = 0;
        int suc = 0;
        while(!q.isEmpty()) {
        ...
        print_util(vssds, allocs, "improved_util.txt");
    }
